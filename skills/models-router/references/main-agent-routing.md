...
- **Effort DOES reach a Workflow sub-agent — measured 2026-07-27.** Four trials at `low` and four at `max`, same model and task. The load-bearing reading is the harness-recorded one: output tokens separated 49k mean at `low` against 118k at `max`. The agents' own `printenv CLAUDE_EFFORT` reports, which matched the requested level, corroborate it — and two agents reported `UNSET` while taking fewer turns than the rest, which is what self-reported fields do. So the stage-1 arm labels are earned. **Do not quote the ratio.** The trial puzzle turned out to have no solution, and an unsolvable task has no stopping condition except giving up, so the numbers measure *persistence* rather than reasoning — a different quantity, not a skewed one. The direction survives (identical fixture in both arms); the magnitude does not.
- **In stage 2 specifically, effort was asserted rather than verified.** The bullet above covers stage 1's Workflow arms, where `CLAUDE_EFFORT` was readable from inside. Stage 2's headless `claude -p --effort …` runs were never probed that way, so "medium" and "high" there rest on the API honouring the flag. Stated narrowly on purpose: after the correction in the previous bullet, "no effort field in the transcripts" is no longer a reason to call anything unverifiable — it is only a reason to say *this* population was not checked.
